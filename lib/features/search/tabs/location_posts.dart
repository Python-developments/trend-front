...
class LocationsPosts extends StatelessWidget {
  List<List<PostModel>> categorizePostsByLocation(List<PostModel> posts) {
    // Initialize a list to store the categorized posts (list of lists).
    List<List<PostModel>> categorizedPosts = [];
    ...
    for (var post in posts) {
      ...
      if (!encounteredLocations.contains(post.locationName)) {
  ...
  Widget build(BuildContext context) {
    ...
    return GetBuilder<ExploreController>(builder: (controller) {
      List<List<PostModel>> postsGroups =
